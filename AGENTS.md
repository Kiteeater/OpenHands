...
- Enterprise org defaults should now be persisted canonically in `enterprise/storage/org.py:agent_settings` as well. Keep projection columns like `default_llm_model`, `default_llm_base_url`, `default_max_iterations`, `agent`, and verification/condenser fields synchronized from that blob for legacy queries/routes. `OrgMember.agent_settings` should persist the member’s full canonical SDK settings blob (Option B / fully independent member settings), while legacy `llm_model`, `llm_base_url`, and `max_iterations` columns are only compatibility projections/fallbacks and must not override keys already present in the canonical JSON.
...
- Enterprise persistence helpers (`OrgStore`, `OrgMemberStore`, `UserStore`, `SaasSettingsStore`, `LiteLlmManager`) should read SDK-owned settings through `settings.agent_settings`, `get_agent_setting(...)`, `get_secret_agent_setting(...)`, and `set_agent_setting(...)` instead of accessing removed flat `Settings` attributes like `llm_model`, `llm_base_url`, `llm_api_key`, or `agent`.
...
- `openhands/storage/data_models/settings.py` keeps one canonical persisted flat blob in `raw_agent_settings` (serialized as `agent_settings`) plus one cached in-memory `AgentSettings` model exposed as `settings.agent_settings`. Keep `to_agent_settings()` only as a compatibility wrapper, use `to_legacy_mcp_config()` only at explicit legacy boundaries, and make sure the `model_validator(mode='before')` preserves both aliased `agent_settings` input and existing `raw_agent_settings` during assignment validation so unrelated field updates do not drop the settings blob.
